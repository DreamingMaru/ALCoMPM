...
Original file is located at
    https://colab.research.google.com/drive/1XJhKWmOp94jgmG3fhNCQy29iyIQEMS4R
"""
...
# slightly modified version of _CalACC for CoMPM
...
class CoMPM_KEMDy20_loader(Dataset):
...
def CoMPM_make_batch(sessions):
    ...
    for session in sessions:
        ...
        batch_input.append(encode_right_truncated(concat_string, bert_tokenizer, max_length = 4095))
...
# remove Self.W terms
# remove W parameter update
# remove context_logit
...
class CoMPM_ERC_model(nn.Module):
    def __init__(self, clsNum):
        ...
        # speaker_model_path = path/model.bin
...
model = CoMPM_ERC_model(clsNum)
model = model.cuda()
model.train()

train_dataset = CoMPM_KEMDy20_loader(train_path, dataclass)
train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, collate_fn=CoMPM_make_batch)
train_sample_num = int(len(train_dataset)*sample)

test_dataset = CoMPM_KEMDy20_loader(test_path, dataclass)
test_dataloader = DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=num_workers, collate_fn=CoMPM_make_batch)

dev_dataset = CoMPM_KEMDy20_loader(dev_path, dataclass)
dev_dataloader = DataLoader(dev_dataset, batch_size=1, shuffle=False, num_workers=num_workers, collate_fn=CoMPM_make_batch)
...
save_path = os.path.join(path, "CoMPM/")
