...
Original file is located at
    https://colab.research.google.com/drive/1AWCHiH-Uc3O4PSE_4p1nfQoPWurZfkp1
"""
    ...
    def __init__(self, txt_file, dataclass):
        ...
        for i, data in enumerate(dataset):
            ...
            if data == '\n' and len(self.dialogs) > 0:
                ...
                context_speaker = []
                continue
            speaker, utt, emo, w2v = data.strip().split('\t')
            context.append(utt)
...
def ALCoMPM_make_batch(sessions):
  ...
  for session in sessions:
      ...
      for turn, (speaker, utt) in enumerate(zip(context_speaker, context)):
          ...
          if turn<len(context_speaker)-1 and speaker == now_speaker:
              speaker_utt_list.append(encode_right_truncated(utt, roberta_tokenizer, max_length = 511)) #PM값
        
      concat_string = inputString.strip()
      batch_input.append(encode_right_truncated(concat_string, bert_tokenizer, max_length = 4095)) ##CoM값
        
      label_ind = label_list.index(emotion) #글자 emotion을 해당하는 integer 찾아서 가져옴
  ...
  batch_input_tokens = padding(batch_input, bert_tokenizer) #CoM값 + Padding
